...
function EZUI:AddKeyAuth(flowConfig)
    if type(flowConfig) == "string" or type(flowConfig) == "number" then
        flowConfig = { Flow = tostring(flowConfig) }
    end
    ...
    if saveKey and isfile and readfile then
        pcall(function()
            local path = (isfolder and isfolder("EZUI_Configs")) and ("EZUI_Configs/" .. fileName) or fileName
            if isfile(path) then
                enteredKey = readfile(path):gsub("%s+", "")
            end
        end)
    end
    ...
    self:AddButton(authTab, "Submit Key", function()
        ...
        self:Notify({ Title = "Key System", Text = "Checking key...", Type = "Info", Duration = 2 })
        self:ValidateKey(enteredKey, { Flow = flow, ScriptId = scriptId }, function(isValid, msg)
            if isValid then
                if saveKey and writefile then
                    pcall(function()
                        if isfolder and not isfolder("EZUI_Configs") then makefolder("EZUI_Configs") end
                        local path = (isfolder and isfolder("EZUI_Configs")) and ("EZUI_Configs/" .. fileName) or fileName
                        writefile(path, enteredKey)
                    end)
                end
                self:Notify({ Title = "Access Granted", Text = "Key Verified! Welcome.", Type = "Info" })
                self.CurrentScreen = "main"
                self.CurrentTabIndex = 1
                self.SelectedIndex = 1
                self.ScrollOffset = 0
    ...
    self.KeyAuthAutoCheck = function(readyCallback)
        if enteredKey and #enteredKey > 0 then
            self:ValidateKey(enteredKey, { Flow = flow, ScriptId = scriptId }, function(isValid)
                if isValid then
                    self.CurrentScreen = "main"
                    if onSuccess then pcall(function() onSuccess(enteredKey) end) end
                else
...
function EZUI:Init()
    ...
    if self.KeySystem then
        self:AddKeyAuth(self.KeySystem)
    end

    if self.KeyAuthAutoCheck then
        self.KeyAuthAutoCheck(function()
            self:_buildHeaders()
            self:_buildTabContent()
            self:_bindKeys()
        end)
